...
Conduct a meticulous, evidence-bound audit of this repository; then, only as the user's chosen mode permits, resolve the confirmed dead through independently-reviewed, build-verified change-sets.
...
1. **Platform detection comes first.** Detect the platform and run only the matching command variants: PowerShell on Windows (5.1 has no `&&`, so use `if` forms), bash on POSIX. Never run the wrong variant.
...
6. **Git preconditions.** Note the repository state. A dirty tree does **not** block the audit, since the Audit Loop is read-only (Principle 1), but it will block Phase 6 (§III Branch Policy). Tell the user early if resolution is going to stall on it.
...
**The mode fork, where the Audit Loop ends:**
...
In both modes, the repository is still byte-for-byte untouched at the end of Phase 5. That is Principle 1: structural, not aspirational.
