...
Named after the ferryman of the dead: every candidate must pay its toll of evidence before crossing, the possibly-living wait on the shore for your judgment, and Charon never kills anyone. He only ferries what has already died.
...
- [Reading the Manifest](#reading-the-manifest)
...
5. **Report.** Writes the **Manifest of the Dead**: summary counts, per-candidate evidence with file and line, clone clusters with a suggested canonical copy, unused dependencies, and coverage gaps for anything the sweep could not check.

## What it will never do

Charon is advisory only. It does not delete code, does not edit source files, does not remove dependencies, and does not "clean up while it's at it." Removals belong in your normal review process, one change set at a time, using the Manifest as input. This is deliberate: reachability analysis has well-known blind spots, and the cost of deleting one reflection-invoked handler exceeds the benefit of auto-deleting a hundred true positives.
...
Good moments to run it: before a large refactor (so you refactor the living, not the dead), after landing a big feature or migration (when superseded code tends to linger), and periodically on long-lived projects. On small codebases a run is cheap; let the size of the Manifest tell you your cadence.
...
- **Candidates for Crossing** carry full evidence and no false-positive flags. Review them, then remove through your normal process.
- **Awaiting Judgment** resemble dead code but match a class the tools systematically misjudge. The flagged class tells you exactly what to check before deciding.
...
Charon is standalone, but if it detects a [Phanes](https://github.com/Aloim/phanes) installation it cooperates with it: the Manifest is filed into the Phanes documentation tree with a proper header, open items land in the session summary's TODO list, and dead exported APIs are drafted as registry annotation proposals for the architect agent, so the agent team stops routing new work onto them. Charon respects Phanes' single-writer rules throughout; it proposes, it never writes anyone else's artifacts.
